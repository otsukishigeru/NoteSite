...
# -*- coding: utf-8 -*-
"""
noteidx_previous7.pdf に #135 エントリを追加して noteidx.pdf を生成する
"""
...
def build_cover_pdf() -> bytes:
    ...
    story = [
        ...
        Spacer(1, 4 * mm),
        Paragraph('1_s_o の note 作品を 135 件を作成日順に掲載します。', cover_sub),
        Spacer(1, 2 * mm),
        Paragraph('（第135番追記：2026 年 6 月 28 日）', cover_sub),
    ]
...
def build_last_page(page_number: int) -> bytes:
    ...
    # 135（新規追記）— KeepTogether で途中改ページを防ぐ
    story += make_entry(no_style, meta_style, url_style, body_style,
        ...
        '三層の分離と連結を提示する。人間の認識階層や自由エネルギー原理を評価プロセスに組み込み、'
        '自己評価を行う内省ループによって知識体系を自律的に改善・進化させる設計思想を論じる。'
...
def main():
    ...
    src  = os.path.join(base_dir, 'noteidx_previous7.pdf')

...
# -*- coding: utf-8 -*-
"""
noteidx_previous5.pdf に #133 エントリを追加して noteidx_previous6.pdf を生成する
  1. 新カバーページ（全 133 件）を生成
  2. 既存ページ index 1〜16（記事 1〜129）をそのまま保持
  3. 最終ページ（130〜132番）を再生成し 133 を追記
  4. pypdf で結合して noteidx_previous6.pdf として保存
"""
...
def build_cover_pdf() -> bytes:
    ...
    story = [
        ...
        Spacer(1, 4 * mm),
        Paragraph('1_s_o の note 作品を 133 件を作成日順に掲載します。', cover_sub),
        Spacer(1, 2 * mm),
        Paragraph('（第133番追記：2026 年 5 月 26 日）', cover_sub),
    ]
...
def build_last_page(page_number: int) -> bytes:
    ...
    # 133（新規追記）— KeepTogether で途中改ページを防ぐ
    story += make_entry(no_style, meta_style, url_style, body_style,
        133,
        '知働化コミュニティへの転回',
        '2026-05-26　♥  3',
        'https://note.com/ichi_s_otsuki/n/n0aad8c53c12d',
        'アジャイルプロセス協議会の終了に伴い、知働化研究会が「知働化コミュニティ」として'
        '独立・転回する背景と今後の方向性を論じる。'
...
def main():
    ...
    src  = os.path.join(base_dir, 'noteidx_previous5.pdf')
    dest = os.path.join(base_dir, 'noteidx_previous6.pdf')
    ...
    print(f'noteidx_previous6.pdf 生成完了: {total_out} ページ')
    print(f'出力先: {dest}')

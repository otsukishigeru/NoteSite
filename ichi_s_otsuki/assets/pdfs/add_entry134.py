...
# -*- coding: utf-8 -*-
"""
noteidx_previous6.pdf に #134 エントリを追加して noteidx.pdf を生成する
  1. 新カバーページ（全 134 件）を生成
  2. 既存ページ index 1〜16（記事 1〜129）をそのまま保持
  3. 最終ページ（130〜133番）を再生成し 134 を追記
...
def build_cover_pdf() -> bytes:
    ...
    story = [
        ...
        Spacer(1, 4 * mm),
        Paragraph('1_s_o の note 作品を 134 件を作成日順に掲載します。', cover_sub),
        Spacer(1, 2 * mm),
        Paragraph('（第134番追記：2026 年 6 月 5 日）', cover_sub),
    ]
...
def build_last_page(page_number: int) -> bytes:
    ...
    # 134（新規追記）— KeepTogether で途中改ページを防ぐ
    story += make_entry(no_style, meta_style, url_style, body_style,
        134,
        '自由エネルギー原理と『思考の技法 2.0』',
        '2026-06-05　♥  1',
        'https://note.com/ichi_s_otsuki/n/nf0120cd3f914',
        'カール・フリストンの「自由エネルギー原理」を『思考の技法』の視点から読み解き、'
        '脳の認識や意思決定の計算モデルとその哲学的背景を論じる。'
        '思考の技法の中心概念である「目的・コミュニケーション・抽象化」や、'
        'オートポイエーシス論との関連を再考している。'
        '脳科学の「大統一理論」が、今後のAIや組織・システム設計にもたらす'
        '新たな可能性を探求する実践的論考。',
        last=True,
        keep_together=True
    )
...
def main():
    ...
    src  = os.path.join(base_dir, 'noteidx_previous6.pdf')

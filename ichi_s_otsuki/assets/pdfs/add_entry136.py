...
# -*- coding: utf-8 -*-
"""
noteidx_previous8.pdf に #136 エントリを追加して noteidx.pdf を生成する
"""
...
def build_cover_pdf() -> bytes:
    ...
    story = [
        ...
        Spacer(1, 4 * mm),
        Paragraph('1_s_o の note 作品を 136 件を作成日順に掲載します。', cover_sub),
        Spacer(1, 2 * mm),
        Paragraph('（第136番追記：2026 年 7 月 9 日）', cover_sub),
    ]
...
def build_last_page(page_number: int) -> bytes:
    ...
    # 136（新規追記）— KeepTogether で途中改ページを防ぐ。
    # 135番の直後に収まらない場合は自動的に改ページされる。
    story += make_entry(no_style, meta_style, url_style, body_style,
...
def main():
    ...
    src  = os.path.join(base_dir, 'noteidx_previous8.pdf')

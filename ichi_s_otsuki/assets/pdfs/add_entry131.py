...
# -*- coding: utf-8 -*-
"""
noteidx_previous3.pdf に #131 エントリを追加して noteidx.pdf を生成する
  1. 新カバーページ（全 131 件）を生成
  2. 既存ページ index 1〜16（記事 1〜129）をそのまま保持
  3. 最終ページ（130番）を再生成し 131 を追記
...
def build_cover_pdf() -> bytes:
    ...
    story = [
        ...
        Spacer(1, 4 * mm),
        Paragraph('1_s_o の note 作品 全 131 件を作成日順に掲載します。', cover_sub),
        Spacer(1, 2 * mm),
        Paragraph('（第131番追記：2026 年 5 月 11 日）', cover_sub),
    ]
...
def build_last_page(page_number: int) -> bytes:
    ...
    # 131（新規追記）— KeepTogether で途中改ページを防ぐ
    story += make_entry(no_style, meta_style, url_style, body_style,
        131,
        '思考の技法 2.0',
        '2026-05-11　♥ –',
...
def main():
    ...
    src  = os.path.join(base_dir, 'noteidx_previous3.pdf')
